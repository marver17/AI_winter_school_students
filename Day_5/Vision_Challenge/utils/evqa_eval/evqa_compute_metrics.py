...
_VOCAB_PATH = 'utils/evqa_eval/vocab.txt' #'gs://cloud-tpu-checkpoints/bert/keras_bert/uncased_L-12_H-768_A-12/vocab.txt'
# TFHUB_CACHE_DIR="/leonardo_scratch/large/userexternal/fcocchi0/reag/hf_models/BERT" python -c "import tensorflow_hub as hub; hub.load('https://tfhub.dev/google/answer_equivalence/bem/1')"
_MODEL_PATH = './hf_models/BERT/8c79027db3c48563b1b91f255e6259f44b118a73' # 'https://tfhub.dev/google/answer_equivalence/bem/1'
...
def compute_score_evqa(input_path: str):

    with open(input_path, 'r') as f:
      data = json.load(f)

    # Prepare score lists for all three eval types (including errors)
    single_hop_scores_all = []



    for element in tqdm(data, mininterval=1, desc='Evaluating'):
      ...
      if element['question_type'] != '2_hop':
        single_hop_scores_all.append(score)


    # Compute final scores
    single_hop_score = np.mean(single_hop_scores_all) if single_hop_scores_all else 0.0

    return single_hop_score
